...
**트레이드오프:** Coast 인스턴스 간 데이터 격리가 없습니다. 모든 인스턴스가 동일한 데이터베이스를 읽고 씁니다. 인스턴스별 데이터베이스가 필요하다면 대신 `strategy = "isolated"`를 사용하는 [볼륨 전략](../concepts_and_terminology/VOLUMES.md)을 사용하거나, 공유 Postgres 안에서 인스턴스별 데이터베이스를 얻기 위해 공유 서비스에 `auto_create_db = true`를 사용하세요. 자세한 내용은 [Shared Services Coastfile reference](../coastfiles/SHARED_SERVICES.md)를 참고하세요.
...
이는 대규모 모노레포에서 가장 영향이 큰 최적화입니다. 매 assign마다 실행되는 gitignored 파일 동기화(rsync)와 `git ls-files` diff 중에 Coast가 전체 디렉터리 트리를 건너뛰도록 지시합니다.
