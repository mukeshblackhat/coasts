...
- **找不到 worktree** — 如果 Coasts 预期某个 worktree 应该存在但无法找到它，请确认 Coastfile 的 `worktree_dir` 包含
  `~/.cursor/worktrees/<project-name>`，并且 `<project-name>` 与
  `~/.cursor/worktrees/` 下的实际文件夹名称一致。有关语法和路径类型，请参见
