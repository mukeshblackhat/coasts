...
- **Worktree не найден** — Если Coasts ожидает, что worktree существует, но не
  может его найти, проверьте, что `worktree_dir` в Coastfile включает
  `.claude/worktrees`. Сведения о синтаксисе и типах путей см. в [Worktree Directories](../coastfiles/WORKTREE_DIR.md).
