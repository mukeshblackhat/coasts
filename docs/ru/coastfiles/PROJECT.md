...
Каталог, где создаются git worktrees для экземпляров Coast. По умолчанию `".worktrees"`. Во время выполнения Coast автоматически определяет каталог из существующих git worktrees (через `git worktree list`) и предпочитает его значению по умолчанию. Относительные пути разрешаются относительно корня проекта.
