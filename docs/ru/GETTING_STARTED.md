...
> **Совет:** После установки Coasts с помощью команд выше вы можете поручить настройку проекта вашему агенту для кодинга. Скопируйте промпт ниже и вставьте его в чат, инициализированный в корневом каталоге проекта, который вы настраиваете.
...
Вы также можете назвать его `Coastfile.toml` для подсветки синтаксиса в редакторе — обе формы эквивалентны. Если существуют оба файла, вариант с `.toml` имеет приоритет.
