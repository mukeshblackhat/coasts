...
Поскольку каждый экземпляр Coast bind-монтирует один и тот же каталог хоста, все они используют одни и те же inode'ы на диске. Это ключевая особенность, которая делает возможной мгновенную синхронизацию файлов, — но это также означает, что блокировки на уровне файлов конфликтуют между экземплярами.

Например, Next.js 16 захватывает эксклюзивный `flock` на `.next/dev/lock` при запуске dev-сервера. Поскольку `flock` — это блокировка ядра на уровне inode, второй экземпляр Coast, пытающийся запустить `next dev` на том же корне проекта, видит блокировку первого экземпляра и немедленно завершается. Та же проблема относится к любому инструменту, который использует `flock`, блокировки `fcntl` или PID-файлы в рабочем пространстве.

Поле `private_paths` в Coastfile решает эту проблему, предоставляя каждому экземпляру свой собственный каталог для указанных путей. После монтирования `/workspace` с общей пропагацией Coast bind-монтирует каталог конкретного экземпляра из `/coast-private/` поверх каждого объявленного пути:

```text
/workspace/frontend/.next  ←──mount──  /coast-private/frontend/.next
```

`/coast-private/` находится в собственной файловой системе контейнера DinD — не на общем монтировании хоста, — поэтому каждый экземпляр естественным образом получает отдельные inode'ы. Монтирования приватных путей повторно применяются при `coast start`, `coast assign` и `coast unassign`.

Подробности конфигурации см. в [`private_paths` в справочнике по Coastfile](../coastfiles/PROJECT.md) и на странице концепции [Private Paths](PRIVATE_PATHS.md).
...
| `/coast-private/*` | bind mount (in-container) | RW | Приватные каталоги для конкретного экземпляра для путей, объявленных в `private_paths`. |
