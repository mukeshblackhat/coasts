...
Это самая влиятельная оптимизация для больших монорепозиториев. Она говорит Coast пропускать целые деревья каталогов во время синхронизации игнорируемых git файлов (rsync) и diff `git ls-files`, которые запускаются при каждом assign.
