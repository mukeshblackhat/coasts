...
各 Coast インスタンスは同じホストディレクトリを bind-mount するため、ディスク上ではすべて同じ inode を共有します。これは即時ファイル同期を可能にする根本的な機能ですが、同時にファイルレベルのロックがインスタンス間で衝突することも意味します。

たとえば Next.js 16 は、dev サーバー起動時に `.next/dev/lock` に対して排他的な `flock` を取得します。`flock` は inode レベルのカーネルロックであるため、同じプロジェクトルートに対して `next dev` を起動しようとする 2 つ目の Coast インスタンスは、1 つ目のインスタンスのロックを見て即座に終了します。同じ問題は、ワークスペース内で `flock`、`fcntl` ロック、または PID ファイルを使うあらゆるツールに当てはまります。

Coastfile の `private_paths` フィールドは、指定したパスに対して各インスタンス専用のディレクトリを与えることで、この問題を解決します。共有伝播付きで `/workspace` をマウントした後、Coast は宣言された各パスの上に `/coast-private/` からインスタンスごとのディレクトリを bind-mount します。

```text
/workspace/frontend/.next  ←──mount──  /coast-private/frontend/.next
```

`/coast-private/` は共有ホストマウント上ではなく DinD コンテナ自身のファイルシステム上に存在するため、各インスタンスは自然に別々の inode を持ちます。private path マウントは `coast start`、`coast assign`、`coast unassign` の際に再適用されます。

設定の詳細については、[Coastfile リファレンスの `private_paths`](../coastfiles/PROJECT.md) と [Private Paths](PRIVATE_PATHS.md) の概念ページを参照してください。
...
| `/coast-private/*` | bind mount (in-container) | RW | `private_paths` で宣言されたパス向けの、インスタンスごとのプライベートディレクトリ。 |
