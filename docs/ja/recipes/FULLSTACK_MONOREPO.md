...
`[healthcheck]` セクションは、各ポートの健全性をどのようにプローブするかを Coast に伝えます。ヘルスチェックパスが設定されたポートでは、Coast は 5 秒ごとに HTTP GET を送ります — どの HTTP 応答でも healthy とみなされます。ヘルスチェックパスがないポートは TCP 接続チェック（接続を受け付けられるか？）にフォールバックします。
...
これは大規模モノレポにおける最も影響の大きい最適化です。毎回の assign で実行される gitignore 対応のファイル同期（rsync）と `git ls-files` の差分で、ディレクトリツリー全体をスキップするよう Coast に指示します。

目的は、Coast のサービスが必要としないものをすべて除外することです。30,000 ファイルのモノレポでは、上記のディレクトリが 8,000+ ファイルを占め、それらは実行中のサービスには無関係かもしれません。除外すれば、ブランチ切り替えごとにそれだけのファイル stat を削減できます。
...
実行中サービスにマウントされるソースコードや、そのサービスがインポートする共有ライブラリを含むディレクトリは残してください。それ以外 — ドキュメント、CI 設定、ツール、他チームのアプリ、モバイルクライアント、CLI ツール、そして `.yarn` のような同梱キャッシュ — は除外します。
